...
const Right3 = (props) => {
  /*
  Right의 최하위 컴포넌트에서는 Click이벤트를 통해 부모쪽에서 전달된 함수를 호출.
  그러면 Right3 -> Right2 -> App과 같은 순서대로 호출됨.
  즉, +버튼을 누르면 State인 number가 1증가되면서 새롭게 렌더링됨.
  */
  return(
    <div>
      <h2>Right3</h2>
...
/*
App 컴포넌트로부터 전달받은 Props를 자식컴포넌트로 재전달.
실제 애플리케이션은 이와같이 중첩된 구조의 UI를 가지게되므로 여러 Depth의 컴포넌트 구조를 가지게된다.
*/
const Left1 = (props) => {
  return(
    <div>
  ...
  return (
    <div className="root">
      ...
      <div id='grid'>
        ...
        {/* Right 컴포넌트 하위로는 State를 변경하는 함수를 전달.
